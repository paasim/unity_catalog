...
static size_t GetRequestWriteCallback(void *contents, size_t size, size_t nmemb,
                                      void *userp) {
...
static string GetRequest(const string &url, const string &token = "") {
  ...
  if (curl) {
    ...
    if (res != CURLcode::CURLE_OK) {
      ...
      throw IOException("Curl Request to '%s' failed with error: '%s'", url,
                        error);
    }
    return readBuffer;
  }
...
template <class TYPE, uint8_t TYPE_NUM,
          TYPE (*get_function)(duckdb_yyjson::yyjson_val *obj)>
static TYPE TemplatedTryGetYYJson(duckdb_yyjson::yyjson_val *obj,
                                  const string &field, TYPE default_val,
...
static uint64_t TryGetNumFromObject(duckdb_yyjson::yyjson_val *obj,
                                    const string &field,
                                    bool fail_on_missing = true,
                                    uint64_t default_val = 0) {
  return TemplatedTryGetYYJson<uint64_t, YYJSON_TYPE_NUM,
                               duckdb_yyjson::yyjson_get_uint>(
      obj, field, default_val, fail_on_missing);
}
static bool TryGetBoolFromObject(duckdb_yyjson::yyjson_val *obj,
                                 const string &field,
                                 bool fail_on_missing = false,
                                 bool default_val = false) {
  return TemplatedTryGetYYJson<bool, YYJSON_TYPE_BOOL,
                               duckdb_yyjson::yyjson_get_bool>(
      obj, field, default_val, fail_on_missing);
}
static string TryGetStrFromObject(duckdb_yyjson::yyjson_val *obj,
                                  const string &field,
                                  bool fail_on_missing = true,
                                  const char *default_val = "") {
  return TemplatedTryGetYYJson<const char *, YYJSON_TYPE_STR,
                               duckdb_yyjson::yyjson_get_str>(
      obj, field, default_val, fail_on_missing);
}
...
public:
  UCAPIError() {}
  UCAPIError(const string &error_code, const string &message)
      : error_code(error_code), message(message) {}

public:
  bool HasError() { return !error_code.empty(); }

public:
  void ThrowError(const string &prefix) {
    D_ASSERT(HasError());
    throw InvalidInputException("%s. error_code: %s, message: %s", prefix,
                                error_code, message);
  }
...
static string GetCredentialsRequest(const string &url, const string &table_id,
                                    const string &token = "") {
  CURL *curl;
  CURLcode res;
  string readBuffer;
  ...
  string body = StringUtil::Format(
      R"({"table_id" : "%s", "operation" : "READ_WRITE"})", table_id);
...
void UCAPI::InitializeCurl() { SelectCurlCertPath(); }
...
string UCAPI::GetDefaultSchema(const UCCredentials &credentials) {
  ...
  duckdb_yyjson::yyjson_doc *doc =
      duckdb_yyjson::yyjson_read(api_result.c_str(), api_result.size(), 0);
...
UCAPITableCredentials
UCAPI::GetTableCredentials(const string &table_id,
                           const UCCredentials &credentials) {
  UCAPITableCredentials result;

  auto api_result = GetCredentialsRequest(
  ...
  auto error = CheckError(root);
  if (error.HasError()) {
    error.ThrowError(StringUtil::Format(
        "Failed to get table credentials for table_id: %s", table_id));
  }
  ...
  if (aws_temp_credentials) {
    ...
    result.secret =
        TryGetStrFromObject(aws_temp_credentials, "secret_access_key");
    result.session_token =
        TryGetStrFromObject(aws_temp_credentials, "session_token");
  }

  return result;
}

vector<string> UCAPI::GetCatalogs(const string &catalog,
                                  const UCCredentials &credentials) {
  throw NotImplementedException("UCAPI::GetCatalogs");
}

static UCAPIColumnDefinition
ParseColumnDefinition(duckdb_yyjson::yyjson_val *column_def) {
...
vector<UCAPITable> UCAPI::GetTables(const string &catalog, const string &schema,
                                    const UCCredentials &credentials) {
  vector<UCAPITable> result;
  auto api_result =
      GetRequest(credentials.endpoint +
                     "/api/2.1/unity-catalog/tables?catalog_name=" + catalog +
                     "&schema_name=" + schema,
                 credentials.token);
  ...
  yyjson_arr_foreach(tables, idx, max, table) {
    ...
    table_result.data_source_format =
        TryGetStrFromObject(table, "data_source_format", false);
    table_result.storage_location =
        TryGetStrFromObject(table, "storage_location", false);
...
vector<UCAPISchema> UCAPI::GetSchemas(const string &catalog,
                                      const UCCredentials &credentials) {
