...
UCCatalog::UCCatalog(AttachedDatabase &db_p, const string &internal_name,
                     AttachOptions &attach_options, UCCredentials credentials,
                     const string &default_schema)
    : Catalog(db_p), internal_name(internal_name),
      access_mode(attach_options.access_mode),
      credentials(std::move(credentials)), schemas(*this),
...
UCCatalog::CreateSchema(CatalogTransaction transaction,
  ...
  if (schema_lookup.GetEntryName() == DEFAULT_SCHEMA &&
    ...
    if (default_schema.empty()) {
      throw InvalidInputException(
          "Attempting to fetch the default schema - but no database was "
          "provided by the catalog");
    }
    return GetSchema(transaction, default_schema, if_not_found);
  }
  auto entry =
      schemas.GetEntry(transaction.GetContext(), schema_lookup.GetEntryName());
...
bool UCCatalog::InMemory() { return false; }

string UCCatalog::GetDBPath() { return internal_name; }

string UCCatalog::GetDefaultSchema() const { return default_schema; }

DatabaseSize UCCatalog::GetDatabaseSize(ClientContext &context) {
  if (default_schema.empty()) {
    throw InvalidInputException(
        "Attempting to fetch the database size - but no database was provided "
...
void UCCatalog::ClearCache() { schemas.ClearEntries(); }

PhysicalOperator &UCCatalog::PlanCreateTableAs(ClientContext &context,
...
PhysicalOperator &UCCatalog::PlanInsert(ClientContext &context,
                                        PhysicalPlanGenerator &planner,
                                        LogicalInsert &op,
  ...
  if (!table.internal_attached_database) {
    ...
    AttachInfo info;
    info.name = "__uc_catalog_internal_" + internal_name + "_" +
                table.schema.name + "_" + table.name; // TODO:
  ...
  if (table_data->storage_location.find("file://") != 0) {
    ...
    auto table_credentials =
        UCAPI::GetTableCredentials(table_data->table_id, credentials);
...
PhysicalOperator &UCCatalog::PlanDelete(ClientContext &context,
                                        PhysicalPlanGenerator &planner,
                                        LogicalDelete &op,
                                        PhysicalOperator &plan) {
...
PhysicalOperator &UCCatalog::PlanUpdate(ClientContext &context,
                                        PhysicalPlanGenerator &planner,
                                        LogicalUpdate &op,
                                        PhysicalOperator &plan) {
...
UCCatalog::BindCreateIndex(Binder &binder, CreateStatement &stmt,
