...
UCTableEntry::UCTableEntry(Catalog &catalog, SchemaCatalogEntry &schema,
...
unique_ptr<BaseStatistics> UCTableEntry::GetStatistics(ClientContext &context,
                                                       column_t column_id) {
  return nullptr;
}
...
void UCTableEntry::BindUpdateConstraints(Binder &binder, LogicalGet &,
                                         LogicalProjection &, LogicalUpdate &,
                                         ClientContext &) {
  throw NotImplementedException("BindUpdateConstraints");
}

TableFunction
UCTableEntry::GetScanFunction(ClientContext &context,
  ...
  auto catalog_entry =
      schema.GetEntry(data, CatalogType::TABLE_FUNCTION_ENTRY, "delta_scan");
  if (!catalog_entry) {
  ...
  auto delta_scan_function =
      delta_function_set.functions.GetFunctionByArguments(
  ...
  if (table_data->storage_location.find("file://") != 0) {
    ...
    auto table_credentials = UCAPI::GetTableCredentials(table_data->table_id,
  ...
  TableFunctionBindInput bind_input(inputs, param_map, return_types, names,
                                    nullptr, nullptr, delta_scan_function,
                                    empty_ref);

  auto result =
      delta_scan_function.bind(context, bind_input, return_types, names);
