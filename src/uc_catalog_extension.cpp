...
static unique_ptr<BaseSecret> CreateUCSecretFunction(ClientContext &,
                                                     CreateSecretInput &input) {
  // apply any overridden settings
  vector<string> prefix_paths;
  auto result =
      make_uniq<KeyValueSecret>(prefix_paths, "uc", "config", input.name);
  for (const auto &named_param : input.options) {
    ...
    if (lower_name == "token") {
      ...
      throw InternalException(
          "Unknown named parameter passed to CreateUCSecretFunction: " +
          lower_name);
    }
  }
...
unique_ptr<SecretEntry> GetSecret(ClientContext &context,
  ...
  // use case
  auto secret_entry =
      secret_manager.GetSecretByName(transaction, secret_name, "memory");
  if (secret_entry) {
    return secret_entry;
  }
  secret_entry =
      secret_manager.GetSecretByName(transaction, secret_name, "local_file");
  if (secret_entry) {
    return secret_entry;
  }
  return nullptr;
...
UCCatalogAttach(optional_ptr<StorageExtensionInfo> storage_info,
  ...
  for (auto &entry : info.options) {
    ...
    if (lower_name == "type" || lower_name == "read_only") {
      ...
      throw BinderException("Unrecognized option for UC attach: %s",
                            entry.first);
    }
  }
  ...
  if (secret_entry) {
    ...
    const auto &kv_secret =
        dynamic_cast<const KeyValueSecret &>(*secret_entry->secret);
    ...
    credentials.aws_region =
        endpoint_val.IsNull() ? "" : aws_region_val.ToString();
...
CreateTransactionManager(optional_ptr<StorageExtensionInfo> storage_info,
...
static void LoadInternal(ExtensionLoader &loader) {
  ...
  CreateSecretFunction mysql_secret_function = {"uc", "config",
                                                CreateUCSecretFunction};
  SetUCSecretParameters(mysql_secret_function);
  loader.RegisterFunction(mysql_secret_function);

  auto &config = DBConfig::GetConfig(loader.GetDatabaseInstance());
  config.storage_extensions["uc_catalog"] =
      make_uniq<UCCatalogStorageExtension>();
}

void UcCatalogExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }
...
DUCKDB_CPP_EXTENSION_ENTRY(uc_catalog, loader) { duckdb::LoadInternal(loader); }
}
