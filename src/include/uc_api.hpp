...
public:
  ...
  static UCAPITableCredentials
  GetTableCredentials(const string &table_id, const UCCredentials &credentials);
  static string GetDefaultSchema(const UCCredentials &credentials);
  static vector<string> GetCatalogs(const string &catalog,
                                    const UCCredentials &credentials);
  static vector<UCAPITable> GetTables(const string &catalog,
                                      const string &schema,
                                      const UCCredentials &credentials);
  static vector<UCAPISchema> GetSchemas(const string &catalog,
                                        const UCCredentials &credentials);
  static vector<UCAPITable> GetTablesInSchema(const string &catalog,
                                              const string &schema,
                                              const UCCredentials &credentials);
};
} // namespace duckdb
