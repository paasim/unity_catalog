...
struct UCTableInfo {
  UCTableInfo() { create_info = make_uniq<CreateTableInfo>(); }
  ...
  UCTableInfo(const SchemaCatalogEntry &schema, const string &table) {
    create_info =
        make_uniq<CreateTableInfo>((SchemaCatalogEntry &)schema, table);
  }
...
public:
  UCTableEntry(Catalog &catalog, SchemaCatalogEntry &schema,
